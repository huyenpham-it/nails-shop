		...
		<div className="product-details w-full max-w-[1100px] flex flex-col md:flex-row justify-center gap-[40px] p-[40px] md:p-[80px] m-auto">
			<div className="w-full md:w-1/2">
				<div className="aspect-square">
					<Image src={product.src} width={300} height={300} alt={product.name} className="w-64" />
				</div>
				<div className="grid grid-cols-4 md:grid-cols-5 gap-2">
					<div className="aspect-square border border-(--color-my-green)">
						<Image src={product.src} width={100} height={100} alt={product.name} className="w-64" />
					</div>
				</div>
			</div>
			...
			<div className="w-full md:w-1/2 flex flex-col gap-6">
				...
				<div className="text-[16px]">{product.description}</div>
			</div>
		</div>
	);
}
